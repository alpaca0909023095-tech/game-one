// 遊戲基礎數值設定檔
// 修改此檔後，重新整理 test.html 或 iframe 即可套用新數值。
// 開關類參數統一使用 1 / 0：1 代表開啟，0 代表關閉。
window.GAME_CONFIG = {
  player: {
    fingerOffsetY: 50,        // 手指觸控點和玩家飛機的垂直距離，數字越大飛機越靠上。
    maxHp: 100,              // 玩家最大血量。
    radius: 24,              // 玩家碰撞半徑，越大越容易撞到敵人。
    startYRate: 0.72,        // 玩家起始 Y 位置比例，0.72 代表畫面高度 72% 的位置。
    followStrength: 200,     // 玩家追向觸控目標的力量，越大移動越靈敏。
    followDamping: 15,       // 玩家移動阻尼，越大越快停下、手感越緊。
  ...
  bullet: {
    ...
    hitSplashColor: "rgba(255,255,255,0.98)", // 普通子彈命中方塊顏色，透明度越高越明顯。
    hitSplashSpeedX: 58,     // 普通子彈命中方塊水平飛濺速度，越小越慢。
    hitSplashSpeedUp: 70,    // 普通子彈命中方塊向上飛濺速度，越小越慢。
    hitSplashSpeedDown: 26,  // 普通子彈命中方塊向下飛濺速度，越小越慢。
    trailInterval: 0,        // 普通子彈尾端白色方塊產生間隔，0 代表關閉。
    trailSizeMin: 6,         // 普通子彈尾端白色方塊最小尺寸。
    trailSizeMax: 9,         // 普通子彈尾端白色方塊最大尺寸。
    trailColor: "rgba(255,255,255,0.98)" // 普通子彈尾端方塊顏色，透明度越低越弱。
  },
  ...
  enemy: {
    hp: 9,                   // 敵人血量。
    minSpeed: 90,            // 敵人最慢下落速度。
    maxSpeed: 150,           // 敵人最快下落速度。
    radius: 29,              // 敵人碰撞半徑。
    hitShakeTime: 0.14,      // 敵人被擊中時震動持續時間。
    hitShakePower: 8,        // 敵人被擊中時震動強度。
    spawnMinInterval: 1.0,   // 敵人生成最短間隔。
    spawnMaxInterval: 1.4,   // 敵人生成最長間隔。
    spawnLevelGrowth: 0.5    // 每過一關敵人生成量增加比例，0.5 代表約增加一半。
  },

  level: {
    duration: 20,            // 每一關持續時間，秒數到後進入升級選擇。
  ...
  upgradeUi: {
    ...
    skillAppear: {           // 技能是否有機率出現在升級 UI，1 代表會出現，0 代表不出現。
      shockwave_1: 0,        // 衝擊波。
      homing_egg_1: 1,       // 追蹤蛋。
      speed_shoot_2: 0,      // 加速射擊。
      fire_team_3: 1,        // 火力班。
      shield_3: 1,           // 護盾。
      shield_amplifier_1: 0  // 護盾放大器；仍需先擁有護盾才會出現。
    }
  },
  ...
  skills: {
    ...
    shockwave: {
      ...
      burstParticleTrackSpread: 2, // 消失粒子離衝擊波軌道的隨機距離，越小越貼近圓環。
      burstParticleSpeedMin: 6,    // 消失粒子向外飄散的最小速度。
      burstParticleSpeedMax: 18,   // 消失粒子向外飄散的最大速度。
      burstParticleDrift: 5,       // 消失粒子的額外亂飄速度，越小越不會擴太出去。
    ...
    shield: {
      ...
      orbitSpeed: 2.8,         // 護盾平均繞主角旋轉速度，數字越大轉越快。
      orbitSpeedVariance: 0.08, // 每顆護盾轉速差異比例，0.08 代表最多約正負 8%。
      size: 18,                // 護盾方塊顯示尺寸。
      ...
      shadowColor: "#38d6bf", // 護盾陰影顏色，目前不使用發光。
      shadowBlur: 0,           // 護盾發光模糊程度，0 代表純色無螢光。
      selfRotateSpeed: 5.33,   // 護盾方塊自轉速度，數字越大轉越快。
      amplifierSizeMultiplier: 1.25 // 護盾放大器的尺寸與碰撞半徑倍率，1.25 代表增加 1/4。
    }
  }
};
